...
import { X, Car, Plus, Trash2, Save, AlertCircle, ChevronDown, ChevronRight } from 'lucide-react';

interface RepairOrderFormModalProps {
  onClose: () => void;
  onSuccess: () => void;
}
...
export default function RepairOrderFormModal({ onClose, onSuccess }: RepairOrderFormModalProps) {
  ...
  const getRepairTypeStyle = (type: RepairType | null) => {
    switch (type) {
      case 'sua_chua': return 'bg-orange-100 text-orange-700 border-orange-200';
      case 'dong_son': return 'bg-sky-100 text-sky-700 border-sky-200';
      default: return '';
    }
  };
                ...
                return (
                  <div key={item.id} className="space-y-1">
                    <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-xl">
                      ...
                      <span className="flex-1 font-medium text-gray-800 text-sm">{item.name}</span>
                      {item.repairType && (
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-lg border ${getRepairTypeStyle(item.repairType)}`}>
                          {item.repairType === 'sua_chua' ? 'SC' : 'DS'}
                        </span>
                      )}
                      {hasSubItems && (
                      ...
                      <div className="ml-8 space-y-1">
                        {item.subItems.map((subItem, subIndex) => (
                          <div key={subItem.id} className="flex items-center gap-2 p-2 bg-white border border-gray-200 rounded-lg">
                            ...
                            <span className="flex-1 text-sm text-gray-700">{subItem.name}</span>
                            <button
                              type="button"
